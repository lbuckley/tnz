...
#BASED ONLY ON EDGES
## ADD SPECIES FORMERLY DROPPED BASED ON UCT QUALITY
#########################################
...
phy=read.csv("Phy_all_wConstrained_noUCTdrop.csv")
phy= phy[phy$Quality %in% c("C","D"),]
...
#write out
write.csv(output, paste(wd, "OUT/BirdTminTmax_noUCTdrop.csv", sep=""), row.names=F)
...
#write out
wd="C:\\Users\\Buckley\\Google Drive\\Buckley\\Work\\TNZ\\"
write.csv(output, paste(wd, "OUT\\MammalTminTmax_noUCTdrop.csv", sep=""), row.names=F)
